-- *** Begin Patch
*** Add File: server/redis_scripts/craft_atomic.lua
+-- craft_atomic.lua
...
+    ],
+    "tokenCost": "0.05",
+    "output": { "itemId": "repair_kit", "qty": 1 },
+    "cooldownSeconds": 60,
+    "requiresLevel": 1,
+    "maxPerDay": 10
+  },
...
+    "materials": [
+      { "itemId": "herb", "qty": 3 },
+      { "itemId": "chem_vial", "qty": 1 }
+    ],
+    "tokenCost": "0.00",
+    "output": { "itemId": "stimpak", "qty": 1 },
+    "cooldownSeconds": 30,
+    "requiresLevel": 1,
+    "maxPerDay": 20
+  },
...
+      { "itemId": "circuit", "qty": 2 }
+    ],
+    "tokenCost": "0.25",
+    "output": { "itemId": "turret_part", "qty": 1 },
+    "cooldownSeconds": 300,
+    "requiresLevel": 3,
+    "maxPerDay": 3
+  },
...
+    "name": "Advanced Kit",
+    "materials": [
+      { "itemId": "turret_part", "qty": 2 },
+      { "itemId": "rare_alloy", "qty": 1 }
+    ],
+    "tokenCost": "1.00",
+    "output": { "itemId": "advanced_kit", "qty": 1 },
...
*** End Patch
*** Begin Patch
*** Add File: scripts/addCraftables.js
+// scripts/addCraftables.js
+// Merge new recipes into data/recipes.json (idempotent)
+const fs = require('fs');
...
+  console.error('Usage: node scripts/addCraftables.js <new_recipes.json>');
+  process.exit(2);
+}
+
+const newPath = path.resolve(process.argv[2]);
+if (!fs.existsSync(newPath)) {
+  console.error('File not found:', newPath);
+  process.exit(2);
+}
+
+const existing = readJson(recipesPath);
+const incoming = readJson(newPath);
+
+// index existing by id
+const map = {};
+existing.forEach(r => { if (r && r.id) map[r.id] = r; });
+
+// merge: update existing ids, add new ones
+incoming.forEach(r => {
+  if (!r || !r.id) return;
+  map[r.id] = Object.assign({}, map[r.id] || {}, r);
+});
+
+// produce sorted array (stable)
+const merged = Object.keys(map).sort().map(k => map[k]);
+
+// backup old file
+const backupPath = recipesPath + '.bak.' + Date.now();
+fs.copyFileSync(recipesPath, backupPath);
+console.log('Backup saved to', backupPath);
+
+// write merged
+writeJson(recipesPath, merged);
+console.log('recipes.json updated. Total recipes:', merged.length);
+
*** End Patch
*** Begin Patch
*** Add File: test/run_lua_test.js
+// test/run_lua_test.js
+// Run this to test the craft_atomic.lua script directly via Redis.
+// Usage: node test/run_lua_test.js
...
+
+async function main() {
+  const wallet = 'TEST_WALLET_1';
+  const playerKey = `player:${wallet}`;
+  // Seed player with materials
+  const player = { lvl: 5, inventory: { scrap_metal: 20, cloth: 10, herb: 10, chem_vial: 5 } };
+  await redis.set(playerKey, JSON.stringify(player));
+  console.log('Seeded player:', playerKey);
+
+  // Load recipe (example)
+  const recipesPath = path.join(__dirname, '..', 'data', 'recipes.json');
+  const recipes = JSON.parse(fs.readFileSync(recipesPath, 'utf8'));
+  const recipe = recipes.find(r => r.id === 'repair_kit') || recipes[0];
+  console.log('Using recipe:', recipe.id);
+
+  // Load Lua script
+  const luaPath = path.join(__dirname, '..', 'server', 'redis_scripts', 'craft_atomic.lua');
+  const script = fs.readFileSync(luaPath, 'utf8');
+
+  // Ensure script loaded to Redis and get SHA
+  const sha = await redis.script('load', script);
+  console.log('Loaded Lua SHA:', sha);
+
+  // Call EVALSHA
+  const keys = [`player:${wallet}`, `craft:cd:${wallet}:${recipe.id}`];
+  const args = [JSON.stringify(recipe), '1', String(Date.now())];
+
+  const res = await redis.evalsha(sha, keys.length, ...keys, ...args);
+  const parsed = JSON.parse(res);
+  console.log('Lua result:', parsed);
+
+  // Show updated player
+  const updated = JSON.parse(await redis.get(playerKey));
+  console.log('Updated player inventory:', updated.inventory);
+
+  await redis.quit();
+}
+
...
*** End Patch
*** Begin Patch
*** Add File: test/e2e_craft_with_txsig.js
+// test/e2e_craft_with_txsig.js
+// Simulate calling your /api/craft endpoint after a client-signed token transfer.
+// Replace TEST_WALLET_PUBLIC_KEY and TEST_TXSIG with real values for devnet testing.
...
+
+async function main() {
+  const wallet = process.env.TEST_WALLET_PUBLIC_KEY || 'TEST_WALLET_PUBLIC_KEY';
+  const recipeId = 'repair_kit';
+  const amount = 1;
+
+  // Optionally seed player inventory in Redis so materials exist
+  const playerKey = `player:${wallet}`;
+  const player = { lvl: 5, inventory: { scrap_metal: 20, cloth: 10 } };
+  await redis.set(playerKey, JSON.stringify(player));
+  console.log('Seeded player in Redis for e2e test');
+
+  const txSig = process.env.TEST_TXSIG || 'REPLACE_WITH_REAL_TXSIG';
+
+  const body = {
+    wallet,
+    recipeId,
...
+  };
+
+  console.log('Calling /api/craft with body:', body);
+  const resp = await fetch(`${API_BASE}/api/craft`, {
+    method: 'POST',
...
+
+  const updated = JSON.parse(await redis.get(playerKey));
+  console.log('Player after craft:', updated);
+
+  await redis.quit();
+}
+
