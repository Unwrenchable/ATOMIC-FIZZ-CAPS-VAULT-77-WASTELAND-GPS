...
(function () {
  ...
  async function loadAppearanceOptions() {
    try {
      console.log('[CharacterCreator] Loading appearance options from /data/character_creator/appearance_options.json');
      const response = await fetch('/data/character_creator/appearance_options.json');
      console.log('[CharacterCreator] Fetch response status:', response.status);
      appearanceOptions = await response.json();
      console.log('[CharacterCreator] Loaded appearance options:', Object.keys(appearanceOptions));
    } catch (error) {
  ...
  const CharacterCreator = {
    ...
    async init() {
      console.log('[CharacterCreator] Starting initialization');
      await Promise.all([
        loadGrokAvatars(),
    ...
    open(existingCharacter, onSave) {
      console.log('[CharacterCreator] Opening character creator');
    ...
    _createOverlay() {
      console.log('[CharacterCreator] Creating overlay');
    ...
    _renderAppearance() {
      console.log('[CharacterCreator] Rendering appearance options');
      if (!this.appearanceOptions) {
        console.log('[CharacterCreator] No appearance options loaded');
        return;
      }

      const container = this.overlayEl.querySelector('#cc-appearance-container');
      console.log('[CharacterCreator] Container found:', !!container);
      if (!container) return;

      // Group options by category
      const categories = {
        face: ['skinTones', 'faceShapes', 'noseTypes', 'mouthTypes'],
        hair: ['hairStyles', 'hairColors'],
        eyes: ['eyeShapes', 'eyeColors'],
        details: ['scars', 'markings', 'accessories']
      };

      container.innerHTML = Object.entries(categories).map(([categoryName, categoryKeys]) => {
        console.log(`[CharacterCreator] Rendering category ${categoryName} with keys:`, categoryKeys);
        const options = categoryKeys.flatMap(key => {
          const opts = this.appearanceOptions[key] || [];
          console.log(`[CharacterCreator] ${key}: ${opts.length} options`);
          // Map JSON key to appearance property name
          const categoryMap = {
            skinTones: 'skinTone',
          ...
          const category = categoryMap[key] || key.replace('s', '').toLowerCase();
          return opts.map(opt => ({ ...opt, category }));
        });
        console.log(`[CharacterCreator] Total options for ${categoryName}: ${options.length}`);
        return `
          <div class="cc-appearance-category">
            <h4>${categoryName.charAt(0).toUpperCase() + categoryName.slice(1)}</h4>
            <div class="cc-appearance-options">
              ${options.map(option => {
                const isSelected = currentCharacter.appearance?.[option.category] === option.id;
                return `
                  <div class="cc-appearance-option ${isSelected ? 'selected' : ''}"
                       onclick="Game.modules.CharacterCreator._selectAppearanceOption('${option.category}', '${option.id}')">
                    ${option.name}
                  </div>
                `;
              }).join('')}
    ...
    _selectAppearanceOption(category, optionId) {
  ...
  console.log('[CharacterCreator] Module loaded, exporting to Game.modules.CharacterCreator');
