...
function saveState(){ try{ localStorage.setItem('af_state', JSON.stringify({
...
function loadState(){ try{ const s=JSON.parse(localStorage.getItem('af_state')||'null'); if(s){
...
function setStatus(t,ms=2500){ const el=document.getElementById('status'); el.textContent=`Status: ${t}`;
...
async function claimLoot(loc) {
  ...
  try {
    const r = await fetch(`${API_BASE}/claim-survival`, {method:'POST',headers:{'Content-Type':'application/json'},
